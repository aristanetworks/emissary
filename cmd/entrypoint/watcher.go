...
func (sh *SnapshotHolder) K8sUpdate(
	...
	changed, err := func() (bool, error) {
		...
		if IsSDSEnabled() {
			...
			// to avoid exposing unreferenced secrets in the SDS snapshot.
			for _, secret := range sh.k8sSnapshot.Secrets {
				sdsSecret, err := BuildSDSSecret(secret, secret.GetNamespace(), secret.GetName())
				if err != nil {
					dlog.Warningf(ctx, "[WATCHER]: Skipping secret %s/%s for SDS: %v", secret.GetNamespace(), secret.GetName(), err)
					continue
				}
	...
	if endpointsChanged || dispatcherChanged || secretsChanged {
