    ...
    for listener in conf["static_resources"]["listeners"]:
        for filter_chain in listener["filter_chains"]:
            for f in filter_chain["filters"]:
                http2_options = f["typed_config"].get("http2_protocol_options", {})
                max_concurrent_streams = http2_options.get("max_concurrent_streams", None)
                assert (
                    max_concurrent_streams is not None
                ), f"max_concurrent_streams not found in http2_protocol_options: {f['typed_config']}"
