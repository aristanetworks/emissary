...
func update(
	...
	if len(sdsSecretsV3) > 0 {
		dlog.Infof(ctx, "Loaded %d SDS secrets for SDS snapshot", len(sdsSecretsV3))
	}
	...
	*generation++
	...
	// Secrets are served exclusively via the dedicated SDS server, not ADS.
	// Envoy will never request SecretType over the ADS stream, so we omit them here.
